...
public class IssuersIntegrationTest {
	...
	public void testGetIssuer() throws IOException, URISyntaxException, InterruptedException {
		final ResponseOrError<Issuer> allResponse = issuers.get(TEST_ISSUER);

		assertThat(allResponse.getSuccess(), is(true));

		final Issuer issuer = allResponse.getData();
		assertThat(issuer.getResource(), is("issuer"));
		assertThat(issuer.getId(), is(TEST_ISSUER));
		assertThat(issuer.getName(), is("Triodos Bank"));
	...
	public void testGetIssuers() throws IOException, URISyntaxException, InterruptedException {
		final ResponseOrError<Page<Issuer>> allResponse = issuers.all(Optional.empty(), Optional.empty());

		assertThat(allResponse.getSuccess(), is(true));

		final Page<Issuer> all = allResponse.getData();
		assertThat(all.getCount(), is(10));
		assertThat(all.getTotalCount(), is(11));
		...
		assertThat(all.getLinks().getNext().isPresent(), is(true));

		final Set<String> identifiers = all.getData().stream().map(Issuer::getId).collect(Collectors.toSet());
